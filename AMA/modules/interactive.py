...
def interactive_shell(chan,main_ins,host_ip,username):   # add:host_ip,username
    if has_termios:
        posix_shell(chan,main_ins,host_ip,username)      # add:host_ip,username
    else:
        windows_shell(chan)
...
def posix_shell(chan,self,host_ip,username):
    ...
    try:
        ...
        cmd_list = []
        cmd = ''
        #c_time = time.strftime("%F")
        #f = file("audit_%s" %c_time,'a+')
