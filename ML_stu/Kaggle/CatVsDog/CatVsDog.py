...
TRAIN_DIR = r'D:\kaggle_data\CatVsDog\train\\'
TEST_DIR = r'D:\kaggle_data\CatVsDog\test\\'
...
print("Train shape: {}".format(train_normalized.shape))     # Train shape: (2000, 150, 150, 3)
print("Test shape: {}".format(test_normalized.shape))       # Test shape: (500, 150, 150, 3)
...
print ('Training', train_dataset.shape, train_labels.shape)     # Training (1600, 150, 150, 3) (1600,)
print ('Validation', valid_dataset.shape, valid_labels.shape)   # Validation (400, 150, 150, 3) (400,)
print ('Test', test_dataset.shape, test_labels.shape)           # Test (500, 150, 150, 3) (500,)
...
print ('Training set', train_dataset.shape, train_labels.shape)     # Training set (1600, 150, 150, 3) (1600, 2)
print ('Validation set', valid_dataset.shape, valid_labels.shape)   # Validation set (400, 150, 150, 3) (400, 2)
print ('Test set', test_dataset.shape, test_labels.shape)           # Test set (500, 150, 150, 3) (500, 2)
...
with tf.Session(graph=graph) as session:
  ...
  for step in range(num_steps):
    ...
    # print(l, predictions)
    if (step % 50 == 0):
      minibatch_accu = accuracy(predictions, batch_labels)
      validation_accu = accuracy(valid_prediction.eval(), valid_labels)
      # test_accu = accuracy(test_prediction.eval(), test_labels)
      accu_matrix[cnt,] = step, minibatch_accu, validation_accu
      cnt += 1
      print ("Minibatch loss at step", step, ":", l)
      print ("Minibatch accuracy: %.1f%%" % minibatch_accu)
      print ("Validation accuracy: %.1f%%" % validation_accu)
...
plt.xlabel("Epochs")
plt.ylabel("Score %")
plt.title("Learning Curves (Tensorflow)")
plt.plot(x, y1, "b", label="Minibatch accuracy")
plt.plot(x, y2, 'r', label="Validation accuracy")
# plt.plot(x, y3, 'g', label="Test accuracy")
plt.legend()
