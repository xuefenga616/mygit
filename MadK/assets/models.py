...
class Asset(models.Model):
    ...
    expire_date = models.DateField(u'过保修期',blank=True,null=True)
    price = models.FloatField(u'价格',blank=True,null=True)
    business_unit = models.ForeignKey('BusinessUnit',verbose_name=u'所属事业线',blank=True,null=True)
    ...
    update_date = models.DateTimeField(auto_now=True,blank=True) #auto_now无论是添加还是修改对象，都会修改时间
...
class NetworkDevice(models.Model):
    ...
    port_num = models.IntegerField(u'端口个数',blank=True,null=True)
...
class CPU(models.Model):
    ...
    cpu_count = models.IntegerField(u'物理CPU个数')
...
class Disk(models.Model):
    ...
    capacity = models.CharField(u'磁盘容量GB',max_length=64)
    disk_iface_choice = (
        ('SATA','SATA'),
        ('SAS','SAS'),
        ('SCSI','SCSI'),
...
class Tag(models.Model):
    ...
    create = models.ForeignKey('UserProfile')
    create_date = models.DateField(auto_now_add=True)
    def __unicode__(self):
        return self.name
...
class EventLog(models.Model):
    ...
    event_type = models.IntegerField(u'事件类型',choices=event_type_choices)
    asset = models.ForeignKey('Asset')
