...
class Asset(object):
    ...
    def get_asset_id_by_sn(self):
        '''When the client first time reports it's data to Server,it doesn't know it's asset
         id yet,so it will come to the server asks for the asset it first,then report the
         data again'''
        ...
        if data:
            try:
                ...
                if self.mandatory_check(data,only_check_sn=True): #the asset is already exit in DB,just return it's asset id to client
    ...
    def data_inject(self):  #注入数据库
        ...
        if self.__is_new_asset():
            ...
            self.create_asset()
        else:#asset already exist,just update it
    ...
    def _create_server(self):
        ...
        log_msg = "Asset [<a href='/admin/assets/asset/%s' target='_blank'>%s</a>] has been created!" %(self.asset_obj.id,self.asset_obj) #target是重定向
    ...
    def __update_asset_component(self,data_source,fk,update_fields,identify_field=None):
        ...
        try:
            ...
            if hasattr(component_obj,'selected_related'):#this component is reverse m2m relation with Asset model
                objects_from_db = component_obj.selected_related()
                    ...
                    if type(data_source) is list:
                        for source_data_item in data_source:
                            ...
                            if key_field_data_from_source_data:
                                if key_field_data == key_field_data_from_source_data:#find the matched source data for this component,then should compare each field in this component to see if there's any changes since last update
                                    ...
                                    break#must break at last,then if the loop is finished,logic will goes for...else part,then you will know that no source data is matched for by using this key_field_data,that means,this items is lacked from source data,it makes sense when the hardware info got changed. e.g:one of the RAM is broken,then this data will not be reported in reporting data
                                ...
                                print "\033[33;1m Error:cannot find any matches in source data by using key field val [%s],component data is missing in reporting data!\033[0m" %(key_field_data)
                                self.response_msg('error','AssetUpdateWarning',"Cannot find any matches in ")
                        ...
                        for key,source_data_item in data_source.items():
                            ...
                            if key_field_data_from_source_data:
                                ...
                                print "\033[33;1m Warning:cannot find any matches in source data by using key field val [%s],component data is missing in reporting data!\033[0m" %(key_field_data)
                    else:
                        print "\033[31;1m Must be sth wrong,logic should goes to here at all.\033[0m"
    ...
    def __filter_add_or_deleted_components(self,model_obj_name,data_from_db,data_source,identify_field):
        ...
        print "\033[31;1m data_only_in_db:\033[0m",data_only_in_db
        print "\033[31;1m data_only_in_data source:\033[0m",data_only_in_data_source
    ...
    def __compare_component(self,model_obj,fields_from_db,data_source):
        #model_obj:具体一张网卡，fields_from_db:要对比的字段，data_source:客户端对应网卡的数据
        for field in fields_from_db: #[ip,slot...]
            ...
            if val_from_data_source:
                ...
                if val_from_db == val_from_data_source:#this field haven't changed since last update
                    ...
                    db_field = model_obj._meta.get_field(field) #内部方法，难点！！！！
