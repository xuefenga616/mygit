...
def trigger_context(data):
    ...
    for expression_item in expressions:
        expression_obj = monitor.models.TriggerExpression.objects.get(id=expression_item['expression_obj'])
        line = '''<p>service:{service} index:{service_index} operator:{operator} func:{calc_func}
        args:{calc_args} threshold:{threshold} calc_res:{calc_res} real_val:{real_val}
